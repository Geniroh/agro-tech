...
const BarChartCard = ({
    title, subtitle, height = 400, data, fill = "#000", XdataKey, YdataKey,
    dataKey, cellFill = "#ed6c3c", className, Xlabel, Ylabel, barWidth = 70, topLabelDataKey = "",
    totalKey, topBarColor = "#8884d8", Ymax
}: BarChartCardProps) => {
    ...
    const yDomain = [0, Ymax || Math.max(...data.map(item => item[totalKey || dataKey])) * 1];

    return (
        <div className={`bg-myoffwhie shadow-[15px] border-border border-2 rounded-lg border-slate-100 w-full p-5 ${className}`}>
            <span className='text-[#101828] text-[14px] font-semibold'>{title}</span>
            <div className='p-10 relative'>
                <div className={`${totalKey ? "left-[2px]" : "left-[-10px]"} top-[50%] absolute -rotate-90 text-[#475467] text-[12px]`}>{Ylabel}</div>
               
                <div style={{ width: "100%" }}>
                    <ResponsiveContainer width="100%" height={height}>
                        <BarChart
                            data={data}
                            margin={{ top: 5, left: -22, bottom: 5 }}
                        >
                            ...
                            />
                            <Bar dataKey={dataKey} barSize={barWidth} strokeWidth={1} radius={[5, 5, 5, 5]} stackId="a">
                            ...
                            {totalKey && (
                                <Bar dataKey={(dataPoint: any) => dataPoint[totalKey] - dataPoint[dataKey]} fill={topBarColor} barSize={barWidth} stackId="a" radius={[5, 5, 0, 0]}>
                                    {data.map((entry: any, index: number) => (
