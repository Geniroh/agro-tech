  ...
  return (
    <Form {...form}>
      <div className="space-y-6">
        ...
        <FormField control={form.control} name="isGenderFriendly"
            render={({ field }) => (
                <FormItem>
        ...
        {form.watch('isGenderFriendly') === 'true' && (
            <FormField control={form.control} name={`gender_description`}
              render={({ field }) => (
                <FormItem>
                   <FormLabel>Briefly describe how this technology is inclusive of the female gender.</FormLabel>
                    <FormControl>
                        <Textarea {...field} placeholder="Description" rows={4} className='bg-[#fafafa]' />
                    </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <div className='mt-10 flex flex-col gap-y-4'>
          ...
          <Button size="lg" variant="outline" className='text-[16px] leading-[22px] rounded-xl font-semibold' onClick={saveStep}>Save Progress</Button>
        </div>
    </Form>
  );
};
